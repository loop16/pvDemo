...
export default function ChartPanel({ data, symbol, onQuarterLevels, selectedModel, selectedOutcome, overlaySymbol, overlayLevels, levelsSource = 'live' }: { 
  ...
  function resetView() {
    ...
    const to   = data[last].time as any;
    const canSetRange = from != null && to != null && Number.isFinite(+from) && Number.isFinite(+to);
    if (canSetRange) {
      try {
        ts.setVisibleRange({ from, to });
        ts.scrollToRealTime();
  ...
  useEffect(() => {
    ...
    (async () => {
      const prevSegs = allSegsRef.current;
          ...
          if (lines.length) {
            ...
            for (const i of rangeOrder) {
              const r = ranges[i];
                ...
                if (selectedModel !== 'simple' && (scenario === 'NONE' || scenario === 'AUTO')) {
                  const fallback = groupedFull.LONG_TRUE.length
                    ? 'LONG_TRUE'
                    : (groupedFull.SHORT_TRUE.length ? 'SHORT_TRUE' : 'NONE');
                  scenario = fallback as OutcomeKey;
                }
      ...
      if (selectedModel === 'simple') {
        ...
        const shouldUpdate = segs.length > 0 || prevSegs.length === 0;
        if (shouldUpdate) {
      ...
      // Ensure initial render after navigation/redirect settles.
  ...
  }, [chartReady, data, symbol, selectedModel, selectedOutcome, overlayLevels, levelsSource]);
